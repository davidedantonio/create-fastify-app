...
const isValidFastifyProject = async (dir, serviceName) => {
  try {
    ...
    let appExist = await fileExists(path.join(dir, 'src'))
    if (!appExist) {
      throw new Error(`Missing ${path.join(dir, 'src')} folder`)
    }

    let servicesExist = await fileExists(path.join(dir, 'src', 'services'))
    if (!servicesExist) {
      throw new Error(`Missing ${path.join(dir, 'src', 'services')} folder`)
    }

    if (serviceName) {
      let servicesFolderExist = await fileExists(path.join(dir, 'src', 'services', serviceName))
