...
async function bundler (filePath) {
  ...
  try {
    ...
    throw new Error('Can not load the content of the Swagger specification file')
  }

  try {
    parsedContent = parseFileContent(fileContent)
  } catch (err) {
    throw new Error('Can not parse the content of the Swagger specification file')
  }

  try {
    dereferenceJson = await dereference(parsedContent)
  } catch (err) {
    throw new Error('Can not dereference the JSON obtained from the content of the Swagger specification file')
  }

  try {
    bundledJson = await bundle(dereferenceJson)
  } catch (err) {
    throw new Error('Can not bundle the JSON obtained from the content of the Swagger specification file')
  }
